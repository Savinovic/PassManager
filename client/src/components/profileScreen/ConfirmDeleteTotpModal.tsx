...
import { removeTotpSecret, errorReset, successReset } from '../../features/totpSlices/removeTotpSecret';

interface ConfirmDeleteTotpModalProps {
  isOpen: boolean;
...
const ConfirmDeleteTotpModal = (props: ConfirmDeleteTotpModalProps) => {
    ...
    const closeHandler = () => {
      ...
      setTimeout(() => {
        isMounted.current && props.setPasswordToDeleteTotp({ id: '', name: '' })
                  ...
                  >
                    ...
                    <Dialog.Title className="flex items-center justify-between w-full text-2xl text-gray-800">
                      <div className="flex items-center">
                        <h2 className="font-semibold">{tr('confirmDelTotpModalHeader', language)}</h2>
                    ...
                    <div className="flex flex-col w-full my-4">
                      ...
                      <p className="mb-2 text-lg text-center text-gray-800 break-words">
                        {tr('confirmDelTotpModalMessage1', language)}
                        <b>{props.passwordToDeleteTotp.name}</b>?
                      </p>
    
                      <p className="mb-2 text-sm text-center text-gray-600 break-words">
                        {tr('confirmDelTotpModalMessage2', language)}
                      </p>
                    </div>
                    ...
                    <div className="relative flex justify-center w-full h-10 mb-1">
                      ...
                      >
                        <button
                          disabled={loading}
                          type="submit"
                          className="px-4 py-2 mr-2 text-white transition bg-red-700 rounded-full hover:opacity-80 active:scale-95 disabled:transition-opacity disabled:opacity-70 disabled:cursor-default disabled:active:scale-100"
                        >
                          {tr('confirmDelTotpModalSubmit', language)}
                        </button>
                        <button
                          disabled={loading}
                          type="button"
                        ...
                        >
                          {tr('confirmDelTotpModalCancel', language)}
                        </button>
                      </Transition>
...
export default ConfirmDeleteTotpModal;