...
import { SERVER_ERROR, UNAUTHORIZED, USER_DOES_NOT_EXIST, INVALID_ACCESS_CODE, ACCESS_CODE_EXPIRED, USER_ALREADY_LOGGED_IN, USER_EMAIL_ALREADY_EXISTS, } from '../constants/ErrorMessages.js';
// POST - /users/registerSendCode
const registerSendCode = async (req, res) => {
    if (req.cookies?.refreshToken)
        throw createError(409, USER_ALREADY_LOGGED_IN);
    ...
    if (conflictUserEmail)
        throw createError(409, USER_EMAIL_ALREADY_EXISTS);
    ...
    await sendEmail(registerSendCodeMessage(validationResult.email.toLowerCase(), code, req.params.language || availableLanguages[0]));
...
const loginSendCode = async (req, res) => {
    ...
    if (!loginUser)
        throw createError(404, USER_DOES_NOT_EXIST);
    const [code] = voucher_codes.generate({ count: 1, length: 4, charset: '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ' });
    const salt = await bcrypt.genSalt(10);
    const hashedCode = await bcrypt.hash(code, salt);
    loginUser.code = hashedCode;
    await loginUser.save();
    await sendEmail(loginSendCodeMessage(validationResult.email.toLowerCase(), code, req.params.language || availableLanguages[0]));
...
const confirmCode = async (req, res) => {
    ...
    if (!checkCode)
        throw createError(406, INVALID_ACCESS_CODE);
    ...
    if (!accessToken)
        throw createError(500, SERVER_ERROR);
    const refreshToken = await getRefreshToken(checkedUser.id, checkedUser.email);
    if (!refreshToken)
        throw createError(500, SERVER_ERROR);
    checkedUser.code = null;
    checkedUser.refreshTokens = checkedUser.refreshTokens.filter((element) => element.expirationDate > Date.now());
...
const refreshAccessToken = async (req, res) => {
    ...
    jwt.verify(req.cookies.refreshToken, config.JWT_REFRESH_TOKEN_SECRET, async (error, decode) => {
        if (error || checkedUser.id !== decode.id)
            throw createError(401, UNAUTHORIZED);
